...
def mock_video_repository():
    """Mock VideoRepository instance."""
    mock = MagicMock()
    mock.mark_thumbnail_downloaded = MagicMock()
    return mock


def test_thumbnails_downloader(
    mock_file_repo, mock_video_repository, monkeypatch, faker, mock_config
):
    """Downloads thumbnails and updates file repository and video repository."""
    ...
    monkeypatch.setattr("tools.helpers.thumbnails_downloader.ClientSession", mock_session)
    ...
    mock_video_repository.mark_thumbnail_downloaded.assert_called_once()
    call_args = mock_video_repository.mark_thumbnail_downloaded.call_args
...
def test_thumbnails_downloader_errors(
    ...
    mock_video_repository.mark_thumbnail_downloaded.assert_not_called()


def test_thumbnails_downloader_di(
    mock_file_repo, mock_video_repository, faker, monkeypatch, mock_config
):
