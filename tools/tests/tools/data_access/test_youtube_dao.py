...
def test_get_info_video_happy_path(extract_info_mock, faker):
    ...
    sut = YoutubeDAO()
    expected = FakeVideoFactory.build(
        comment_count=faker.pyint(min_value=1, max_value=10),
        like_count=faker.pyint(min_value=1, max_value=10),
        view_count=faker.pyint(min_value=1, max_value=10),
    ).model_dump()
    ...
    assert len(info) == 1
    info_obj = info[0]
    assert isinstance(info_obj, Video)
    assert info_obj.id == expected["id"]
    assert info_obj.comment_count > 0
    assert info_obj.like_count > 0
    assert info_obj.view_count > 0
