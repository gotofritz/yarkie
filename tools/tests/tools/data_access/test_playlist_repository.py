...
class TestGetAllPlaylistsKeys:
    """Tests for get_all_playlists_keys method."""

    def test_returns_enabled_playlists_only(self, db_with_playlists: SQLClient) -> None:
        """Should return only enabled playlist IDs."""
        mock_logger = Mock()
        repository = PlaylistRepository(
            sql_client=db_with_playlists, logger=mock_logger
        )
    ...
    def test_returns_empty_tuple_when_no_playlists(
    ...
    def test_orders_by_last_updated_desc(self, test_sql_client: SQLClient) -> None:
    ...
    def test_logs_error_on_database_failure(self, test_sql_client: SQLClient) -> None:
        ...
        assert "Error retrieving playlist keys" in str(
...
class TestUpdatePlaylists:
    ...
    def test_inserts_new_playlists(self, test_sql_client: SQLClient) -> None:
    ...
    def test_updates_existing_playlists(self, db_with_playlists: SQLClient) -> None:
    ...
    def test_sets_last_updated_timestamp(self, test_sql_client: SQLClient) -> None:
        ...
        with Session(test_sql_client.engine) as session:
            stmt = select(PlaylistsTable).where(
                PlaylistsTable.id == playlists[0].id
            )
            playlist = session.execute(stmt).scalar_one()
    ...
    def test_logs_error_on_database_failure(self, test_sql_client: SQLClient) -> None:
        ...
        mock_logger.error.assert_called_once()
        assert "Error updating playlists" in str(mock_logger.error.call_args[0][0])


class TestClearPlaylistLinks:
    """Tests for clear_playlist_links method."""

    def test_removes_all_links_for_given_playlists(
        self, db_with_playlists: SQLClient
    ) -> None:
        ...
        with Session(db_with_playlists.engine) as session:
            stmt = select(PlaylistEntriesTable).where(
                PlaylistEntriesTable.playlist_id == "playlist1"
            )
        ...
        with Session(db_with_playlists.engine) as session:
            ...
            stmt = select(PlaylistEntriesTable).where(
                PlaylistEntriesTable.playlist_id == "playlist2"
            )
            entries = session.execute(stmt).fetchall()
            assert len(entries) == 1

        mock_logger.info.assert_called_once_with(
            "Removed links to videos (if any) for 1 playlists"
        )

    def test_handles_multiple_playlists(self, db_with_playlists: SQLClient) -> None:
    ...
    def test_handles_empty_list(self, test_sql_client: SQLClient) -> None:
    ...
    def test_handles_playlists_with_no_links(
        self, test_sql_client: SQLClient
    ) -> None:
    ...
    def test_logs_error_on_database_failure(self, test_sql_client: SQLClient) -> None:
        ...
        mock_logger.error.assert_called_once()
        assert "Error clearing playlist links" in str(
            mock_logger.error.call_args[0][0]
        )


class TestCreatePlaylistRepository:
    ...
    def test_creates_repository_instance(self, test_sql_client: SQLClient) -> None:
        ...
        repository = create_playlist_repository(
            sql_client=test_sql_client, logger=mock_logger
        )
